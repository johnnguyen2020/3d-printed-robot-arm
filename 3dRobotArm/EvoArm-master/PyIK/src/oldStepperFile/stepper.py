...
'''this is not how you make a tuple'''
...
'''declare tuple'''
        ...
        for x in range(int(round(myTuple[0][0]/2))):
            GPIO.output((smallStep, midStep, bigStep), GPIO.HIGH)
            sleep(delay)
            GPIO.output((smallStep, midStep, bigStep), GPIO.LOW)
            sleep(delay)
        
        GPIO.output(midDir, midRotate)
        for x in range(int(round((midAngle-smallestAngle)/2))):
            GPIO.output((midStep, bigStep), GPIO.HIGH)
            sleep(delay)
            GPIO.output((midStep, bigStep), GPIO.LOW)
            sleep(delay)

        GPIO.output(bigDir, bigRotate)
        for x in range(int(round((bigAngle-midAngle-smallestAngle)/2))):
            GPIO.output((bigStep), GPIO.HIGH)
            sleep(delay)
            GPIO.output((bigStep), GPIO.LOW)
            sleep(delay)
